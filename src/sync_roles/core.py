...
def sync_roles(
    ...
    with adapter.transaction():
        ...
        # Get database OID that is in database-specific role names
        ...
        existing_permissions = (
            adapter.get_existing_permissions(role_name, preserve_existing_grants_in_schemas)
            if not role_to_create
            else []
        )

        # Compare and determine changes (using helper function to avoid duplication)
        ...
        if logins_to_grant:
            adapter.grant_login(role_name, logins[0])
        if logins_to_revoke:
