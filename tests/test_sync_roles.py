...
from sync_roles import RoleMembership
from sync_roles import SchemaCreate
from sync_roles import SchemaOwnership
from sync_roles import SchemaUsage
from sync_roles import TableSelect
from sync_roles import drop_unused_roles
from sync_roles import sync_roles

from .conftest import ROLES_PER_TEST
from .conftest import ROOT_DATABASE_NAME
from .conftest import TEST_BASE_ROLE
from .conftest import TEST_DATABASE_NAME
from .conftest import engine_future
from .conftest import engine_type
...
def test_many_roles_with_database_connect_does_not_raise_exception(test_engine):
    with test_engine.connect() as conn:
        for role_name in (uuid.uuid4().hex for _ in range(ROLES_PER_TEST)):
            sync_roles(conn, role_name, grants=(DatabaseConnect(TEST_DATABASE_NAME),))
...
def test_if_cannot_choose_database_role_name_runtime_exception_raised(test_engine, monkeypatch):
    ...
    with test_engine.connect() as conn, pytest.raises(RuntimeError, match='Unable to find available role name'):
        sync_roles(conn, role_name, grants=(DatabaseConnect(TEST_DATABASE_NAME),))
...
def test_database_connect_does_not_accumulate_roles(test_engine):
    ...
    with test_engine.connect() as conn_test, test_engine.connect() as conn_sync:
        ...
        sync_roles(conn_sync, role_name, grants=(DatabaseConnect(TEST_DATABASE_NAME),))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Login(valid_until=datetime.now(UTC) + timedelta(minutes=10)),),
...
@pytest.mark.parametrize(
    ...
    [
        lambda _, __: (),
        lambda _, __: (Login(valid_until=datetime(2000, 1, 1, tzinfo=UTC)),),
        lambda _, __: (Login(valid_until=None),),
        lambda _, __: (Login(valid_until=datetime.now(UTC) + timedelta(minutes=10)),),
...
def test_login_is_only_applied_to_passed_role(test_engine, get_valid_until):
    ...
    with test_engine.connect() as conn:
        sync_roles(
            conn,
            role_name_with_login,
            grants=(
                Login(valid_until=get_valid_until(), password='password'),
                DatabaseConnect(TEST_DATABASE_NAME),
            ),
        )
        sync_roles(conn, role_name_without_login, grants=(DatabaseConnect(TEST_DATABASE_NAME),))
...
def test_login_without_database_connect_cannot_connect(test_engine, get_valid_until):
    role_name = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(Login(valid_until=get_valid_until(), password='password'),))
...
def test_login_with_with_connect_then_revoked_cannot_connect(test_engine, get_valid_until_1, get_valid_until_2):
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(conn, role_name, grants=(Login(valid_until=get_valid_until_2(), password='password'),))
...
def test_login_cannot_connect_after_second_sync_with_no_password_and_valid_until_in_past(
    ...
    role_name = get_test_role()
    valid_until_past = datetime.now(UTC) - timedelta(minutes=10)
    with test_engine.connect() as conn:
        sync_roles(
            conn,
            role_name,
...
def test_multiple_login_raises_value_error(test_engine):
    ...
    valid_until = datetime.now(UTC) + timedelta(minutes=10)
    with (
        test_engine.connect() as conn,
        pytest.raises(ValueError, match='At most 1 Login object can be passed via the grants parameter'),
    ):
        sync_roles(
            conn,
            role_name,
...
def test_role_membership_in_one_step(test_engine):
    role_name = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(RoleMembership(TEST_BASE_ROLE),))
...
def test_role_membership_only_granted_to_passed_role(test_engine):
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(conn, role_name_2, grants=(RoleMembership(TEST_BASE_ROLE),))
...
def test_role_membership_only_granted_to_multiple_roles(test_engine):
    ...
    role_name_2 = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name_1, grants=(RoleMembership(TEST_BASE_ROLE),))
        sync_roles(conn, role_name_2, grants=(RoleMembership(TEST_BASE_ROLE),))
...
def test_table_direct_select_does_not_increase_roles(test_engine, test_table, compile_):
    ...
    with test_engine.connect() as conn:
        num_roles_before = conn.execute(sa.text('SELECT count(*) FROM pg_roles')).fetchall()[0][0]

    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(TableSelect(schema_name, compile_(table_name), direct=True),))

        num_roles_after = conn.execute(sa.text('SELECT count(*) FROM pg_roles')).fetchall()[0][0]
...
def test_table_direct_usage_does_not_increase_roles(test_engine, test_table):
    ...
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(SchemaUsage(schema_name, direct=True),))
...
def test_schema_create_direct_does_not_increase_roles(test_engine, test_table):
    ...
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(SchemaCreate(schema_name, direct=True),))
    ...
    with test_engine.connect() as conn:
        sync_roles(
            ...
            grants=(
                ...
                TableSelect(schema_name, compile_(table_name), direct=table_select_direct),
            ),
        )
        sync_roles(
            conn,
    ...
    with test_engine.connect() as conn:
        sync_roles(
            ...
            grants=(
                Login(valid_until=valid_until, password='password'),
                DatabaseConnect(TEST_DATABASE_NAME),
                TableSelect(schema_name, compile_(table_name), direct=direct),
            ),
        )
...
def test_table_select_via_regex_granted_can_query(
    ...
    with test_engine.connect() as conn:
        sync_roles(
            ...
            grants=(
                ...
                TableSelect(
                    schema_name_granted,
                    re.compile(re.escape(table_prefix_granted) + '.*'),
                    direct=table_select_direct,
                ),
            ),
        )
    ...
    with engine.connect() as conn:
        assert (
            conn.execute(
                sa.text(f'SELECT count(*) FROM {schema_name_granted}.{table_prefix_granted + "a"}'),
            ).fetchall()[0][0]
            == 0
        )
        assert (
            conn.execute(
                sa.text(f'SELECT count(*) FROM {schema_name_granted}.{table_prefix_granted + "b"}'),
            ).fetchall()[0][0]
            == 0
        )
    ...
    with (
        ...
        assert (
            conn.execute(
                sa.text(f'SELECT count(*) FROM {schema_name_granted}.{table_prefix_not_granted + "c"}'),
            ).fetchall()[0][0]
            == 0
        )
    ...
    with (
        ...
        assert (
            conn.execute(
                sa.text(f'SELECT count(*) FROM {schema_name_not_granted}.{table_prefix_granted + "a"}'),
            ).fetchall()[0][0]
            == 0
        )
...
def test_table_select_in_two_steps_on_select_can_query(
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(conn, role_with_direct_permissions, grants=(SchemaUsage(schema_name, direct=schema_usage_direct),))
        sync_roles(
            conn,
            role_with_direct_permissions,
            grants=(
                SchemaUsage(schema_name, direct=schema_usage_direct),
                TableSelect(schema_name, compile_(table_name), direct=table_select_direct),
            ),
...
def test_table_select_in_two_steps_on_usage_can_query(
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(
            conn,
            role_with_direct_permissions,
            grants=(TableSelect(schema_name, compile_1(table_name), direct=table_select_direct),),
        )
        sync_roles(
            conn,
            role_with_direct_permissions,
...
def test_table_select_with_usage_in_intermediate_role_can_query(
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(conn, role_name_2, grants=(SchemaUsage(schema_name, direct=schema_usage_direct),))

    engine = sa.create_engine(
        f'{engine_type}://{role_name_1}:password@127.0.0.1:5432/{TEST_DATABASE_NAME}',
        **engine_future,
    )
    with engine.connect() as conn:
        assert conn.execute(sa.text(f'SELECT count(*) FROM {schema_name}.{table_name}')).fetchall()[0][0] == 0
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(
            ...
            grants=(
                ...
                TableSelect(schema_name, compile_(table_name)),
            ),
        )
...
def test_table_select_and_usage_if_owned_by_other_user(
    ...
    with test_engine.connect() as conn:
        assert (
            conn.execute(
                sa.text(f"SELECT nspowner::regrole FROM pg_namespace WHERE nspname='{schema_name}'"),
            ).fetchall()[0][0]
            == role_name_1
        )
    ...
    engine = sa.create_engine(
        f'{engine_type}://{role_name_2}:password@127.0.0.1:5432/{TEST_DATABASE_NAME}',
        **engine_future,
    )
    with engine.connect() as conn:
        assert conn.execute(sa.text(f'SELECT count(*) FROM {schema_name}.{table_name}')).fetchall()[0][0] == 0
...
def test_table_select_granted_can_query_even_if_another_table_not_exists(
    ...
    compile_2,
    schema_usage_direct,
    table_select_direct_1,
    table_select_direct_2,
):
    schema_name, table_name = test_table
    role_name = get_test_role()
    valid_until = datetime.now(UTC) + timedelta(minutes=10)
    ...
    with test_engine.connect() as conn:
        sync_roles(
            ...
            grants=(
                ...
                DatabaseConnect(TEST_DATABASE_NAME),
                SchemaUsage(schema_name, direct=schema_usage_direct),
                TableSelect(schema_name, compile_(table_name), direct=table_select_direct_1),
                TableSelect('does-not-exist', compile_(table_name), direct=table_select_direct_2),
            ),
        )
...
def test_schema_usage_repeated_does_not_increase_role_count(test_engine, test_table, direct):
    ...
    role_name = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(SchemaUsage(schema_name, direct=direct),))
...
def test_table_select_repeated_does_not_increase_role_count(test_engine, test_table, direct, compile_1, compile_2):
    ...
    role_name = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(TableSelect(schema_name, compile_1(table_name), direct=direct),))

        count_roles_1 = conn.execute(sa.text('SELECT count(*) FROM pg_roles')).fetchall()[0][0]

        conn.commit()
        sync_roles(conn, role_name, grants=(TableSelect(schema_name, compile_2(table_name), direct=direct),))
...
def test_schema_ownership_can_be_granted(test_engine, test_table):
    ...
    role_name = get_test_role()
    with test_engine.connect() as conn:
        sync_roles(conn, role_name, grants=(SchemaOwnership(schema_name),))
...
def test_schema_ownership_can_be_revoked(test_engine, test_table):
    ...
    with test_engine.connect() as conn:
        assert conn.execute(
            sa.text(f"SELECT nspowner::regrole::name = CURRENT_USER FROM pg_namespace WHERE nspname='{schema_name}'"),
        ).fetchall()[0][0]
...
def test_schema_ownership_and_usage(test_engine, test_table, direct):
    ...
    with engine.connect() as conn:
        assert (
            conn.execute(
                sa.text(f"SELECT nspowner::regrole::name FROM pg_namespace WHERE nspname='{schema_name}'"),
            ).fetchall()[0][0]
            == role_name
        )
...
def test_direct_table_permission_can_be_revoked_when_not_owner(test_engine, test_table, direct):
    ...
    role_2_engine = sa.create_engine(
        f'{engine_type}://{role_name_2}:password@127.0.0.1:5432/{TEST_DATABASE_NAME}',
        **engine_future,
    )
...
def test_can_use_schema_if_just_created(test_engine, schema_usage_direct, schema_create_direct):
    ...
    with test_engine.connect() as conn:
        ...
        sync_roles(
            conn,
            role_name,
            preserve_existing_grants_in_schemas=(schema_name,),
            grants=(
                Login(valid_until=valid_until, password='password'),
                DatabaseConnect(TEST_DATABASE_NAME),
                SchemaOwnership(schema_name),
...
def test_team_role_privileges_are_preserved(test_engine, schema_usage_direct, schema_create_direct):
    ...
    def sync_role_with_team(role_name):
        with test_engine.connect() as conn:
            ...
            sync_roles(
                conn,
                team_role_name,
                preserve_existing_grants_in_schemas=(team_schema_name,),
                grants=(
    ...
    role_name_1_engine = sa.create_engine(
        f'{engine_type}://{role_name_1}:password@127.0.0.1:5432/{TEST_DATABASE_NAME}',
        **engine_future,
    )
    role_name_2_engine = sa.create_engine(
        f'{engine_type}://{role_name_2}:password@127.0.0.1:5432/{TEST_DATABASE_NAME}',
        **engine_future,
    )
